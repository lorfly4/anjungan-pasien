...
<body class="bg-gray-100 font-sans min-h-screen flex flex-col items-center pt-8">
    ...
    <main class="flex flex-col md:flex-row items-center space-y-6 md:space-y-0 md:space-x-6 w-full max-w-4xl px-4">
        <form method="POST" action="/bpjs/lama" class="flex flex-col space-y-4 flex-grow max-w-lg w-full">
            @csrf
            ...
            <button type="submit" class="bg-blue-500 text-white py-4 rounded-md font-semibold text-xl hover:bg-blue-700">submit</button>
        </form>

        <div class="grid grid-cols-3 gap-2 max-w-xs w-full">
            @for ($i = 1; $i <= 9; $i++)
                <button type="button" onclick="keypad('{{ $i }}')" class="bg-white border border-gray-300 rounded-md py-4 text-xl font-semibold text-gray-700 hover:bg-gray-50">{{ $i }}</button>
            @endfor
            <button type="button" onclick="keypad('0')" class="bg-white border border-gray-300 rounded-md py-4 text-xl font-semibold text-gray-700 hover:bg-gray-50">0</button>
            <button type="button" onclick="hapusInput()" class="bg-red-500 text-white rounded-md py-4 text-xl font-semibold hover:bg-red-700">Hapus</button>
        </div>
    </main>
